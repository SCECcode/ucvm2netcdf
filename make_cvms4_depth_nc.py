...
def read_grid(grd_name,lat_steps,lon_steps,z_steps,lat_size,lon_size,z_size):
    ...
    dataset.title = "A 3D structural velocity model for the southern California crust and upper mantle."
    dataset.id = "SCEC_CVM_S_v4"
    dataset.summary = "CVM-S4 is comprised of detailed basin velocity descriptions based on tens of thousands of direct velocity (Vp, Vs) measurements and incorporates the locations and displacement of major fault zones that influence basin structure. These basin descriptions were used to developed tomographic models of crust and upper mantle velocity and density structure, which were subsequently iterated and improved using 3D waveform adjoint tomography."
    dataset.reference = "Kholer et al. (2003)"
    dataset.references = "http://dx.doi.org/10.1785/0120020017"
    dataset.keywords = "seismic, shear wave, s wave, p wave, density, elastic waveform"
    ...
    dataset.acknowledgment = "Model development was lead by a research team at SCEC and in the Seismological Laboratory at the California Institute of Technology."
    dataset.license = "These data may be redistributed and used without restriction."
    dataset.history = "CVM-S4 was released in 2006. This NetCDF version created " + time.ctime(time.time())
    dataset.comment = "This netCDF model was extracted from CVM-S4 using UCVM and converted to netCDF by SCEC."
    dataset.source = 'SCEC CVM-S4 NetCDF file'
    ...
    dataset.geospatial_vertical_max = 54.0
    ...
    deps.units = 'km'
    deps.positive = 'up'
    deps.standard_name = 'depth'
    deps.long_name = 'depth below earth surface (bes)'
    ...
    #
    # Populate 3 lists of mesh points depth,lat,lon
    # This is discovering the mesh points by looking for repeated
    # coordinates in the files.
    ...
    #print(z_depths[:],len(z_depths))
    #print(y_lats[:],len(y_lats))
    #print(x_lons[:],len(x_lons))
    #sys.exit(0)

    #
    # Convert the single dime array into np array, and then into coordinate variables
    #
...
if __name__ == "__main__":
    ...
    mesh_file_name = "/Users/maechlin/emc_files/cvm4_depth_mesh.txt"
    #
